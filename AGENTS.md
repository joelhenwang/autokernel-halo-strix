...
- **Stack D is the production-recommended recipe (2026-05-11+).** On OdinFlat at +10.7% over Sprint 3A-confirm baseline (34,697 vs 31,331 tok/s aggregate). Compose via `--ak-loss-zloss --ak-fix-rope-gate-op --ak-causal-conv-shim --ak-sync-cleanup --ak-spectra-branchless --ak-normuon-telemetry` on top of existing Sprint 3A recipe. Quality strictly better than plain fused-zloss (Stack A): -0.009 dolma BPB, -0.013 gpt_small BPB, 52% lower max layer kurtosis. See `docs/perf/v3-final-stack-scorecard.md`. Sprint 3 launch: `STACK=D bash scripts/launch_sprint3a.sh` (awaits explicit user approval).
- **v3 granular `--ak-*` flag taxonomy (28 flags):** use granular flags instead of monolithic `--optimize-kernels`. Key flags for Stack D: `--ak-loss-zloss` (fused CE + z-loss), `--ak-fix-rope-gate-op` (T-3.2 rope+gate autograd-safe route), `--ak-causal-conv-shim` (T-3.2 2nd half, clean Dynamo boundary), `--ak-sync-cleanup` (branchless SPECTRA + deferred loss sync), `--ak-normuon-telemetry` (per-param JSONL). Optional diagnostics: `--ak-trust-cap <f>` + `--ak-trust-cap-scope {w_gate_up,spiking,all_2d}`, `--ak-w-gate-up-scale <f>` + `--ak-w-gate-up-ramp-steps <N>`. Full taxonomy in `docs/research/autokernel-40k-v3-execution-plan.md` §5.2.
- **`register_autocast` rule required on all new training-path custom_ops (2026-05-11+).** v3 H11 root cause: Phase B's autograd-safe wiring was insufficient — custom-op autocast boundary dtype drift caused Phase C/G divergence. Fix is `op.register_autocast("cuda", dtype)` with per-op dtype choice: fp16 for homogeneous SwiGLU path ops, fp32 for rotary_emb_fp32, NO rule for mixed-dtype ops like `fused_rope_gate_mul` (hand-managed at call site). See `kernels/hip/_torch_ops.py` for patterns.
- **`--ak-trust-cap` and `--ak-w-gate-up-scale` are diagnostic only** (2026-05-11+). C.2 trust cap probe produced 0 triggers across 2059 telemetry samples on OdinFlat → update-scale is not a primary instability mechanism. Keep for debugging novel instability patterns, but NOT part of Stack D production recipe.
- **Compiled autograd regresses throughput on gfx1151** (2026-05-11+). `--ak-compiled-autograd` ships as infrastructure only; adds -0.6% to -0.5% vs baseline because backward is near-theoretical (T-0.1: 2.18× forward for 2× FLOPs) and gfx1151 has little Python autograd overhead to amortize. Do NOT include in production recipe.
- **batch=32 provides no throughput gain on Strix Halo/gfx1151 for OdinFlat** (2026-05-11+). B.2 measured -0.1% to -0.2% at 24.6 GB memory (vs 13.5 GB at batch=16). Workload is bandwidth/IO-bound at MFU 20.7%, not compute. Stay at batch=16 accum=8 unless memory is a constraint.
- **DDP bucket_cap_mb is null-effect on gfx1151/gloo/TB4** (2026-05-11+). B.1 measured all 4 sizes within 0.67%. Manual `allreduce_grads_async` in train_ddp.py dominates the allreduce path; DDP bucket subsystem sees only 1 event per opt step. Keep default bucket_cap_mb=25. `gradient_as_bucket_view=True` is hardcoded on at train_ddp.py:1403.
