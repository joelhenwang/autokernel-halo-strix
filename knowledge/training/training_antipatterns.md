...
- **Cumulative tok/s inflated by Inductor warmup**: `total_tokens/total_elapsed` rises as early slow compile steps amortize. Reported 63K tok/s when real instantaneous was 26K. Always use **instantaneous** measurement: `interval_tokens / interval_elapsed` between log points. Fixed in `train_ddp.py`.
- **Pre-compile HIP kernels before DDP**: `--optimize-kernels` hangs in DDP when HIP cache is cold (both ranks compile simultaneously → deadlock). Run `python scripts/precompile_kernels.py` on each machine first.
...
- **Width-reduced ablation models are vocab-bottlenecked**: d=384 with 32K vocab: LM head matmul dominates (vocab is 83× larger than d). Use depth-reduced variants (fewer layers at same d=768) for screening. Same GEMM shapes as production, representative throughput ratios. Tested: d=384 4L×2iter gets same tok/s as d=768 2L×1iter — model compute is trivial, vocab matmul + CE loss dominate both.
...
- **Ablation sweet spot: bs=32 accum=2 eager (no compile, no AK)**: At d=768 on Strix Halo, throughput plateaus ~10.3K tok/s eager. Compile adds +7% but costs 25+ min cold cache. `--optimize-kernels` adds +15% but costs 30+ min chunked CE compilation. For 35-min screening runs, **zero compilation overhead beats marginal throughput gains**. Use plain eager.
- **Don't use --optimize-kernels for ablation screening**: Chunked CE HIP kernel takes 30+ min to compile on first invocation. After cache, subsequent runs start instantly — but for a 20-run ablation campaign where each run is 35 min, the first-run penalty is unacceptable. Only use AK for Tier M/V (longer runs amortize compilation).
- **Don't use --compile for ablation screening**: Cold Inductor cache for new model shapes takes 25+ min. Only 7% throughput gain. Not worth it for <1 hour runs. Use for Tier M/V only.
- **Queue ablation runs sequentially, don't wait for results between them**: Run the full Tier S batch (10-20 runs) back-to-back without manual intervention. Analyze results after all complete. Waiting between runs for manual decisions wastes hours.
