    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int i = 0; i < kNItems; ++i) {
            ...
            if constexpr (kDeltaSoftplus) {
                delta_vals[i] = delta_vals[i] <= 20.f ? __builtin_logf(1.0f + __builtin_expf(delta_vals[i])) : delta_vals[i];
            }
        }
        ...
        if constexpr (kHasZ) {
            ...
            for (int i = 0; i < kNItems; ++i) {
                ...
                float z_sigmoid_val = 1.0f / (1.0f + __builtin_expf(-z_val));
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            if constexpr (!kIsComplex) {
                #pragma unroll
                for (int i = 0; i < kNItems; ++i) {
                    const float delta_a_exp = __builtin_exp2f(delta_vals[i] * A_scaled);
        ...
        if constexpr (kDeltaSoftplus) {
            ...
            for (int i = 0; i < kNItems; ++i) {
                ...
                float delta_val_neg_exp = __builtin_expf(-delta_val);
