    ...
    for (int chunk = 0; chunk < params.n_chunks; ++chunk) {
        ...
        for (int r = 0; r < kNRows; ++r) {
            ...
            for (int i = 0; i < kNItems; ++i) {
                ...
                if (params.delta_softplus) {
                    delta_vals[r][i] = delta_vals[r][i] <= 20.f ? __builtin_logf(1.0f + __builtin_expf(delta_vals[r][i])) : delta_vals[r][i];
                }
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            for (int r = 0; r < kNRows; ++r) {
                ...
                for (int i = 0; i < kNItems; ++i) {
                    if constexpr (!kIsComplex) {
                        thread_data[i] = make_float2(__builtin_exp2f(delta_vals[r][i] * A_val[r]),
        ...
        if constexpr (kHasZ) {
            ...
            for (int r = 0; r < kNRows; ++r) {
                ...
                for (int i = 0; i < kNItems; ++i) {
                    ...
                    out_vals[r][i] *= z_val / (1 + __builtin_expf(-z_val));
                }
                __syncthreads();
