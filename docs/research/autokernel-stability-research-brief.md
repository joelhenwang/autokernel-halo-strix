...
**H11: Custom-op autocast boundary mismatch (contributed by external research engineer, v2 response, 2026-05-10).** Our Phase B `torch.library.custom_op` wrappers around HIP kernels register `register_autograd` but NOT `register_autocast`. When the native training path runs under `torch.amp.autocast`, PyTorch autocasts `F.silu` inputs to fp16 with specific dtype semantics. Our custom op may receive inputs in a slightly different dtype state (e.g. autocast-promoted fp32 passed through silu_gate_mul that expects fp16 inside, producing cast-boundary rounding that accumulates across 14 layers × many steps). The native forward would do `silu(gate.to(fp16))*up.to(fp16)` via Inductor's autocast-aware fusion; our custom op does the same math but at a slightly different cast boundary. Over 250-750 steps, accumulated drift could push the update statistics out of the scaled-gradient headroom envelope. **Prediction:** adding explicit `register_autocast(device_type="cuda", cast_inputs=torch.float16)` to all 8 Phase-B custom_ops, OR moving internal sigmoid/silu to fp32 intermediate with fp16 output, should extend the divergence step count by ≥2×. **Co-priority with H4** as the most probable Phase C/G mechanism.

**H14: Optimizer state mismatch after native→optimized switch (contributed by external research engineer, v2 response, 2026-05-10).** When training starts with `--optimize-kernels` on from step 0, the NorMuon momentum and Newton-Schulz internal state is populated by gradients flowing through custom-op paths. If we later warm-start from a native checkpoint that trained through step 500 (our Phase T-3.2 experiment), the saved optimizer state was populated by *native* gradients. Resuming with `--optimize-kernels` active means one-step transition from native-gradient-populated momentum to custom-op-gradient-populated momentum. The momentum terms no longer reflect the new gradient distribution, and the first few post-switch updates could be badly misaligned. **Prediction:** warm-start with *preserved* optimizer/scaler state fails faster than warm-start with *fresh* optimizer/scaler state; the difference diagnoses state mismatch specifically. Our T-3.2 matrix tests exactly this.

**H17: Forward graph breaks fragment backward enough to alter Inductor fusion and reduction ordering (contributed by external research engineer, v2 response, 2026-05-10).** PyTorch's compiled autograd documentation describes how forward graph breaks can propagate to backward graph fragmentation — each unbroken forward segment becomes a separate backward segment, and Inductor cannot fuse across segment boundaries in backward any more than it could in forward. Our T-0.4 inventory found only 2 distinct forward break locations (conv_blocks rope + DaoAILab causal_conv1d), but if these fragment the backward into 3+ segments, the reduction order of `aten::add_` across residual connections could differ from native. In fp16 with non-associative addition, different reduction orders produce bit-different gradients. Over many steps these could accumulate. **Prediction:** fixing the 2 forward breaks (T-1.3) simultaneously reduces divergence likelihood AND enables compiled-autograd (T-2.2). If divergence extends to >1000 steps after T-1.3 fixes, this hypothesis is supported. Weaker than H11 given T-0.1 shows backward is near-theoretical (2.18× forward), suggesting fragmentation impact is already small.
