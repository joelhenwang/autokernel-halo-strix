...
- [ ] **HIP kernels in training paths MUST also use `register_autocast`** (2026-05-11+, v3 H11 fix). `op.register_autocast("cuda", dtype)` with per-op dtype: fp16 for homogeneous SwiGLU-path ops (silu_gate_mul, rmsnorm, fused_res_rmsnorm, causal_conv1d), fp32 for ops expecting fp32 internals (rotary_emb_fp32), NO rule for mixed-dtype ops like fused_rope_gate_mul (hand-managed at call site). Failure to register causes Phase C/G-style divergence in optimized stacks despite autograd being safe. See `kernels/hip/_torch_ops.py` patterns + `docs/perf/dtype-autocast-inventory-post-A3.md`.
- [ ] **Tier 2 parity test required pre-ship** for any new HIP/Triton op in the training path. Template: `scripts/test_tier2_parity.py` (4 ops × {vanilla, autocast} = 8 cells). Forward + backward rel_err <5e-3 under both vanilla fp16 and `torch.amp.autocast(cuda, fp16)` contexts.
- [ ] **Sprint 3A/3B launches require explicit user approval** (universal rule, 2026-05-11+). Agent NEVER auto-launches `bash scripts/launch_sprint3a.sh` or `bash scripts/launch_sprint3b.sh`. Present winning stack + expected throughput + wall-time estimate and WAIT for user go-ahead.
