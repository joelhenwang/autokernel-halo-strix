...
CKPT="${CKPT:-checkpoints/phase-c-odinflat-postfix}"
rm -rf "$CKPT"
mkdir -p "$CKPT"

echo "=== Phase C: OdinFlat 2000-step, post-B-fix, --optimize-kernels + --use-fused-zloss ==="
echo "  CKPT: $CKPT"
...
torchrun --nproc_per_node=1 --nnodes=1 --node_rank=0 \
  ...
  --max-steps 2000 \
  --optimize-kernels --use-fused-zloss \
